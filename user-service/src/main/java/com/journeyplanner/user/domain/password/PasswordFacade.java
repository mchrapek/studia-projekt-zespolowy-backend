...
public class PasswordFacade {
    ...
    public void generateAndSendResetPasswordLinkWithTokenByAdmin(final String email, final String firstName) {
        ResetToken resetToken = resetTokenCreator.from(email);
        resetTokenRepository.save(resetToken);
        log.info(format("New reset token generated by admin : {0} : for email {1}", resetToken.getToken(), email));
        ...
        mailSender.publish(SendMailEvent.builder()
                ...
                .build());
        log.info(format("Mail sent with token by admin : {0} : to user : {1}", resetToken.getToken(), email));
    }
